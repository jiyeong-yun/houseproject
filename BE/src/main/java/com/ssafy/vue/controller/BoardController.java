...
public class BoardController {
	...
	@ApiOperation(value = "게시판 글 총개수", notes = "모든 게시글의 개수를 반환한다.", response = List.class)
	@GetMapping("/allcnt")
	public ResponseEntity<Integer> getTotalCount(@ApiParam(value = "게시글을 얻기위한 부가정보.", required = true) BoardParameterDto boardParameterDto) throws Exception {
	...
	@ApiOperation(value = "댓글보기", notes = "글번호에 해당하는 댓글의 정보를 반환한다.", response = List.class)
	@GetMapping("/memo/{articleno}")
	public ResponseEntity<List<MemoDto>> getMemo(@PathVariable("articleno") @ApiParam(value = "얻어올 글의 글번호.", required = true) int articleno) throws Exception {
		logger.info("getMemo- 호출 : " + articleno);
		
		return new ResponseEntity<List<MemoDto>>(boardService.listMemo(articleno), HttpStatus.OK);
	}
	
	@ApiOperation(value = "댓글 쓰기", notes = "댓글을 쓴다.", response = List.class)
	@PostMapping("/memo")
    ...
    public ResponseEntity<String> deleteMemo(@PathVariable("memono") @ApiParam(value = "살제할 글의 글번호.", required = true) int memono) throws Exception {
